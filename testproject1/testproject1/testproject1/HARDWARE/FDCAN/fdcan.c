...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if((RxFifo0ITs&FDCAN_IT_RX_FIFO0_NEW_MESSAGE)!=RESET)   //FIFO1新数据中断
    {
        ...
        //printf("rxdata[%d]:0x%x\r\n",i,rxdata[i]);
				...
				if (rxdata[0] == 0x9a)
				{
					read_status_1(FDCAN1_RxHeader.Identifier-0x140,rxdata);
				}
				
				if ((rxdata[0] == 0x9c)||(rxdata[0] == 0xa4))
				{
					read_status_2(FDCAN1_RxHeader.Identifier-0x140,rxdata);
				}
				if (rxdata[0] == 0x92)
				{
					read_position(FDCAN1_RxHeader.Identifier-0x140,rxdata);
				}
